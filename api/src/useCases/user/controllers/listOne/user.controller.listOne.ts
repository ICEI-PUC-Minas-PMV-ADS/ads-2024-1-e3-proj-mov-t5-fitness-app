...
import { IListOneUserRes } from '../../dto';
import { UserListOneService } from '../../services';

@Controller('user')
export class UserListOneController {
    constructor(
        private readonly userListOneService: UserListOneService,
    ...
    async listOne(
        @Param('id') id: string,
    ): IResponse<Promise<IResponseError | IResponseSuccess<IListOneUserRes>>> {
        try {
